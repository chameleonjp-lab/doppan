...
`main`へ専用ワークフローを取り込んだ後は、`workflow_dispatch`で同一リポジトリ内の信頼済みrefと表示名を明示して更新できる。現在の確認対象はマージ済み成果を含む`main`で、既存のG1-A / G1-Bおよび過去の統合候補refも後方互換のためallowlistに残す。checkoutしたSHAが対象ブランチの現在tipと一致する場合だけ実行し、`main`のpushでは自動配置しない。`refs/pull/*`とallowlist外のSHAは拒否する。
