...
- 更新日: 2026-08-11
- 正本対象: `main`
- 現在の段階: G0通過 / G0.5未着手 / G1-A自動ゲート通過 / G1-B Draft自動ゲート通過・本人判断待ち
...
| 項目 | 状態 | 内容 |
|---|---|---|
| 計画体系 | 改訂中 | データ非保存とGitHub規約境界を新しい正本へ反映中 |
| G0 制作目的 | 通過 | カメレオンJPの代表作候補として制作 |
| G0.5 市場・名称 | 未着手 | 8〜12作品比較と名称確認 |
| G1-A 技術基盤 | 通過 | 固定環境CIと未告知確認ビルドが通過。Pages実配置と端末確認は節目へ繰り延べ |
| G1-B 物理試作 | Draft自動ゲート通過・本人判断待ち | 最小盤面、入力、物理、状態、品質、Chromium、WebKitの自動ゲートが通過。正式な段階通過はユーザー本人が判断 |
...
既存の`src/storage/save-storage.ts`は、ゲーム記録用の汎用保存として使わない。G1-B開始前または設定実装時に、設定専用の名称、型、試験へ修正する。
...
1. 自動ゲートの残件なし。CI run #13はquality、Chromium、WebKitの全ジョブ成功
2. G1-Bを通過扱いにするかはユーザー本人が判断する
...
1. CI成功済みのG1-B Draft PRについて、ユーザー本人が通過可否を判断する
2. G0.5市場・名称調査とG2中心体験・ショットマップへ進む
