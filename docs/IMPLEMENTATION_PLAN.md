...
- 最終更新日: 2026-08-14
- 版: 5.8
- 状態: G0.5調査完了・G1-A通過・G1-B通過・G2自動ゲート通過・G3/GA技術縦切り・プレイヤー画面マージ済み・安全ショット返球・再発射の固定物理確認済み・Pages確認待ち
...
G3 / GAの3球縦切りはPR #11、プレイヤー向け画面と操作面はPR #16、通常操作から安全ショットまでの因果パスはPR #17、対応フリッパーへの返球・同側実衝突はPR #18、同側入力後の固定物理による上向き再発射はPR #19でマージ済みである。CI Run #43はquality、Chromium、WebKitの全ジョブが成功した。

次の作業単位では、ゲームルールを変えず、`main`の検査済みビルドを手動Pages確認枠へ配置する。

- `Development preview`の既定refを`main`にし、mainへのpushでは自動配置しない
- workflow内でcheckoutしたSHAが`origin/main`の現在tipと一致することを確認する
- ルートには`pages/root/root-guide.html`の案内だけを置く
- ゲームは`/_preview/current/`へ同じworkflowで検査したVite `dist`だけを置く
- ルート案内、ゲーム起動、ビルドSHA、`robots.txt`、非保存境界を実URLで確認する

この自動確認はPages配信経路の正しさを扱う。初見プレイヤーの成功率、左右の戦略差、端末上の操作感、クライマックスの面白さ、正式公開は合格扱いにしない。確認ページとGitHub側の設定が整うまで、管理端末の試遊は開始しない。
