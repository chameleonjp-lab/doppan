...
既存の汎用保存部品は、G1-B開始前または設定画面実装時に、非個人設定専用へ用途を狭める。得点や履歴を受け取れる汎用保存として正式版へ残さない。
...
G1-B開始前に、保存部品の用途を非個人設定専用へ狭める計画を確定する。
...
G1-AのPages・iPhone確認を完了した後、G1-B前に保存部品を非個人設定専用へ狭める。

確認すること:

> 再現可能な依存関係、CI、Chromium・WebKit試験、単一GameLoop、GitHub Pagesの未告知確認枠を成立させ、iPhone Safariから安全に確認できること。ゲーム記録、個人アカウント、外部送信を実装せず、設定保存だけを明確に分離すること。
