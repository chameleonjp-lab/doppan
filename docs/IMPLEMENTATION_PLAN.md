...
- 最終更新日: 2026-08-13
- 版: 5.7
- 状態: G0.5調査完了・G1-A通過・G1-B通過・G2自動ゲート通過・G3/GA技術縦切り・プレイヤー画面マージ済み・安全ショット操作因果パス検証中・Pages確認待ち
...
G3 / GAの3球縦切りはPR #11、プレイヤー向け画面と操作面はPR #16、通常操作から安全ショットまでの因果パスはPR #17でマージ済みである。次は確認ページへ配置する前に、成功後の実球が次の操作位置へ戻ることを物理で証明する。
...
- 成功後の球を直接移動せず、60Hz / 120Hzで対応フリッパーへの実到達と同側Pointer入力の実衝突を確認する
- Chromium / WebKitで画面上の発射・フリッパー操作からHUD更新まで確認する

この作業は実操作での到達可能性と対応フリッパーへの返球を証明するが、同側入力で上向きに再発射できること、初見プレイヤーの成功率、左右の戦略差、クライマックスの面白さを合格扱いにしない。上向き再発射は次の物理調整・試遊で判定する。
