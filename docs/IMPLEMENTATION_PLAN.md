...
- 最終更新日: 2026-08-13
- 版: 5.7
- 状態: G0.5調査完了・G1-A通過・G1-B通過・G2自動ゲート通過・G3/GA技術縦切り・プレイヤー画面マージ済み・安全ショット返球・再発射の固定物理確認中・Pages確認待ち
...
ただし、PR #16までの試験は、発射、フリッパー作動、センサー成功を別々に確認しており、通常操作からショット成功までを一本の因果として証明していなかった。また、`R0`が発射レーンと重なり、フリッパー未入力でも成功し得た。このため、Pages実配置より先に、発射・Pointer入力だけで安全ショット、得点、進行、次目標、物理ゲート変更までを通す回帰試験を追加する。残る確認は、その操作因果パスのCI、Pagesのビルド済み成果物配信、実URLのルート分離、管理端末の試遊である。
...
G3 / GAの3球縦切りはPR #11、プレイヤー向け画面と操作面はPR #16、通常操作から安全ショットまでの因果パスはPR #17、対応フリッパーへの返球・同側実衝突はPR #18でマージ済みである。次は確認ページへ配置する前に、成功後の実球を同側入力で上向きへ再発射できることを物理で証明する。

確認すること:

> 画面相当の最大発射とフリッパーPointer入力だけで安全ショットが成立し、得点、`1 / 5`進行、次目標、戻りゲートが一緒に変わること。

追加確認:

> 成功後の実球が`L0`は左、`R0`は右の対応フリッパー付近へ戻り、同側Pointer入力で実フリッパー衝突が起きること。

追加確認:

> 返球後の同側フリッパー衝突で、実球の上向き速度が正になること。

現在の作業単位では、`L0` / `R0`を発射レーン外の実フリッパー軌道へ置き、次を同じ自動ゲートへ固定する。

- 球の直接移動、速度の直接変更、`ShotCompleted`の直接生成を使わない
- 60Hz / 120Hzで、押下位置と保持時間を組み合わせた各10個の固定入力条件のうち8個以上を成功させる
- 発射だけ、または無関係なフリッパーでは成功させない
- 成功後に得点100、進行`1 / 5`、次目標、対応する戻りゲートを確認する
- 成功後の球を直接移動せず、60Hz / 120Hzで対応フリッパーへの実到達、同側Pointer入力の実衝突、衝突後の上向き速度を確認する
- Chromium / WebKitで画面上の発射・フリッパー操作からHUD更新まで確認する

この作業は実操作での到達可能性、対応フリッパーへの返球、固定物理での上向き再発射を証明するが、初見プレイヤーの成功率、左右の戦略差、端末上の操作感、クライマックスの面白さを合格扱いにしない。

操作因果パスのCIと独立レビュー後は、検査済みビルドをルート案内・プレビュー分離の契約へ接続する。PagesのSource設定、Environment承認、Repository variable、手動workflow実行はGitHub側で確認する。ゲームルール、本番素材、ランキング、保存、外部通信は変更しない。Pages確認後に管理端末の試遊へ進む。
