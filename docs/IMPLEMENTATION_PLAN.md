...
G3 / GAの3球縦切りはPR #11、プレイヤー向け画面と操作面はPR #16でマージ済みである。次は、確認ページへ配置する前に、プレイヤー操作が安全ショットの結果へ届くことを証明する。
...
この作業は実操作での到達可能性を証明するが、初見プレイヤーの成功率、左右の戦略差、成功後の球の戻りが次の入力を変えること、クライマックスの面白さを合格扱いにしない。
