...
- 最終更新日: 2026-08-11
- 版: 5.4
- 状態: G0.5調査完了・G1-A通過・G1-B通過・G2仮設計完了・graybox-alpha実装中
...
G2仮図形の実装範囲と自動検証は[G2仮図形検証記録](./G2_GRAYBOX_ALPHA_VERIFICATION.md)で管理する。自動試験が通っても、初見理解と面白さはG3 / GA未判定である。
...
G1-Bのマージ後は、G2の仮設計をG3 / GAの仮図形へ移す。

確認すること:

> 成功ショットで戻り道が変わり、変化した戻り道が次の狙いとフリッパー操作を変えること。

次の実装単位は`agent/graybox-alpha`を基準とし、G1-Bの物理・入力・状態・安全停止を再利用する。仮図形の実装範囲と自動検証は[G2仮図形検証記録](./G2_GRAYBOX_ALPHA_VERIFICATION.md)へ記録する。新しい本番素材、ランキング、保存、外部通信は含めない。端末確認はGAの最初にまとめる。
