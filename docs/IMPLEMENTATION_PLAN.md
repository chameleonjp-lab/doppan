...
> 2026-09-05：ユーザー本人の根本変更指示により、新しい実装方針は [PACHI_REDESIGN.md](./PACHI_REDESIGN.md) を優先する。本書の3球・経路接続・旧工程の記述は旧版の履歴である。単一ゲームループ、描画と物理の分離、固定更新、GitHub Pagesの静的配信は新実装でも維持する。
...
G3 / GAの技術縦切りとPages配信確認に加え、`R0→L1` / `L0→R1`の中核戻りを実入力で確認した。今回の差分では、上部の危険ルートを先に増やさず、実球が通る下降戻り列へL1/R1を合わせた。次の作業単位ではゲームコードを変更せず、管理端末で人手試遊を行い、G4 / VSへ進める条件を判定する。

1. 3球の一回のゲームを最後まで遊ぶ
2. 初見の発射理解、左右目標の理解、戻り表示、結果・再挑戦を記録する
3. 同じ参加者で3〜5ゲーム遊び、成功率、狙い、次に練習したい行動の変化を記録する
4. `L2` / `R2`以降から`C1`までの実プレイヤー経路と、片側連打で中心機能を完了できないことを確認する
