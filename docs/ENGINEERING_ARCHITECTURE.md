...
球落下、ゲーム終了、重大得点確定など、消してはいけないイベントを`pendingTerminalEvents`へ保存する。
...
上限超過時にゲームルールに必要なデータを捨てない。安全停止し、診断を残す。
...
将来ランキングを追加する場合も、無効状態の記録を送信しない。

通常の手動ポーズや画面非表示による安全停止だけでは、正しく復帰できた場合に記録を無効化しない。
...
- 接触重複
- 資源増加
- GA開始時へ引き継ぐ二本指、Safari、体感遅延、発熱の確認項目
